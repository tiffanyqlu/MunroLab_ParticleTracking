function [Intensities] = getIntensityVsTime(trks)
    ...
    %
        ...
        % compute MSD Vs Tau
        Intensities = cell(numSamples,numParameters);

        for i = 1:numSamples
            for j = 1:numParameters
                numTracks = size(trks{i,j},2);
                Intensities{i,j} = zeros(numTracks,1);
                for k=1:numTracks  
                    Intensities{i,j}(k) = getMSDvsTAU(trks{i,j}(k),trks{i,j}(k).lifetime,info);
                end
                Intensities{i,j} = getCumDvsTAU(trks{i,j}(k),info,sgWindowSize);
            end
        end
end