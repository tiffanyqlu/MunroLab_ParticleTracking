function [CumDVSTau] = getCumDVSTau(trks,info, sgWindowSize)
    ...
    % the Cumulative Distance Vs Tau curves
    ...
    %
        ...
        % compute MSD Vs Tau
        CumDVsTau = cell(numSamples,numParameters);

        for i = 1:numSamples
            for j = 1:numParameters
                CumDVSTau{i,j} = getCumDvsTAU(trks{i,j}(k),info,sgWindowSize);
            end
        end
end