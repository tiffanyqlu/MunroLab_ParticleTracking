...
meanDecays = squeeze(sum(decays,1,'omitnan'));
    ...
    title('mean decay curves');
    saveas(f,[outputFolder '/mean_decay.pdf']);

%%%%%%%%%%%%%  PLOT POOLED DECAY CURVES %%%%%%%%%%%%%
    f = figure;
    if numParameterVals > 1 
        for j = 1:numParameterVals    
            subplot(1,numParameterVals,j); hold;
...
%%
  ...
  if numParameterVals > 1 
    ...
    for j = 1:numParameterVals   
        a_slope(j) = log(pooledDecays(j,minT)) - log(pooledDecays(j,minT+1));
    ...
    a_slope = log(pooledDecays(minT)) - log(pooledDecays(minT+1));
...
save(fullname,"beta"); % saves a file called "beta_(imaging condition).mat" with the fit parameters
...
% plot curves without individual decay curves
    ...
    ylim([-6 0]);
    %xlim([0 4]);
    title('pooled decay curve with fit');
...
% k_d + k_pb == beta_stream(4,1); equation for streaming data
% k_d + 0.05*k_pb == beta_tl(4,1); equation for timelapse data 

% ****fix these for your imaging conditions!

A = [1 1 ; 0.05 1]; 
b = [beta_stream(4,1) ; beta_tl(4,1)];
rates = A\b; % rates: streaming and timelapse
